...
@pytest.fixture
def list_reprint_db(tmp_path: Path) -> Path:
    """Database with PLST and MB1 entries alongside a normal set."""
    payload = {
        "meta": {"date": "2026-03-26", "version": "1.0.0"},
        "data": {
            "M10": {
                ...
                "releaseDate": "2009-07-17",
                "cards": [
                    {"uuid": "bolt-m10-146", "name": "Lightning Bolt", "number": "146", "language": "English", "layout": "normal"},
                    {"uuid": "forest-m10-247", "name": "Forest", "number": "247", "language": "English", "layout": "normal"},
                ],
            },
            "PLST": {
                "code": "PLST",
                ...
                "cards": [
                    {"uuid": "bolt-plst-m10-146", "name": "Lightning Bolt", "number": "M10-146", "language": "English", "layout": "normal"},
                ],
            },
            "MB1": {
                "code": "MB1",
                ...
                "cards": [
                    {"uuid": "forest-mb1-247", "name": "Forest", "number": "247", "language": "English", "layout": "normal"},
                ],
            },
        },
    }
    path = tmp_path / "AllPrintings.list.json"
    path.write_text(json.dumps(payload))
    db_path = tmp_path / "mtgjson.sqlite"
    import_all_printings(source_path=path, db_path=db_path, manifest_path=tmp_path / "manifest.json")
    return db_path
...
def test_check_list_reprint_not_a_list_card(list_reprint_db: Path) -> None:
    index = MTGJSONIndex(list_reprint_db)
    # Forest in M10 is not in PLST (only MB1), but check_list_reprint checks PLST first
    # then MB1. Forest is in MB1 via "247", not via PLST compound key.
    # Use a card that is genuinely absent from all List sets.
    result = index.check_list_reprint(set_code="M10", collector_number="999")
    assert result is None


def test_check_list_reprint_already_list_set_not_checked(list_reprint_db: Path) -> None:
    """Cards already in PLST should not be double-checked (PLST-M10-146 → PLST-PLST-M10-146)."""
    index = MTGJSONIndex(list_reprint_db)
    # Looking up PLST/M10-146 as a compound key would produce "PLST-M10-146" which doesn't exist
    result = index.check_list_reprint(set_code="PLST", collector_number="M10-146")
    assert result is None
